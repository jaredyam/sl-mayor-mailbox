...
class Scraper:
    ...
    def __init__(self):
        ...
        self.total_pages = int(
            self.first_page_soup.find("a", class_="Next").parent.find_all("a")[-2].text
        )
        self.mails_per_page = len(
            self.get_mail_urls_from_page_soup(self.first_page_soup)
        )
    ...
    def get_new_mail_urls(self):
        ...
        total_mails = self.mails_per_page * (self.total_pages - 1) + len(
        ...
        while not found_latest_saved_id:
            ...
            for url in mail_urls:
                ...
                next_page_href = self.current_page_soup.find("a", class_="Next").get(
                    "href"
                )
                current_page_soup = get_soup(
                    self.BASE_URL + self.DEFAULT_QUERY + "?" + next_page_href
                )
    ...
    def get_mail_urls_from_page_soup(self, page_soup):
        ...
        return [
            self.BASE_URL + item.a.get("href") for item in main_table.find_all("tr")[1:]
        ]
    ...
    async def get_mail_content(self, mail_url, session):
        ...
        avaliable_headers = {
            h.text: h for h in soup.find_all("td", class_="titlestyle1335")
        }
        target_headers = ["信件编号", "来信主题", "信件内容", "来信时间", "回复内容", "回复时间", "回复人"]
        ...
        record = dict(zip(record_headers, [None for _ in range(len(record_headers))]))

        for i, th in enumerate(target_headers):
            if th in avaliable_headers:
                record[record_headers[i]] = (
                    avaliable_headers[th].parent.find("td", "contentstyle1335").text
                )
        ...
        if record["reply_agency"] is not None:
            record["reply_agency"] = record["reply_agency"].strip().replace("、", " ")

        print(
            f'saved mail - query date : {record["query_date"]} - '
    ...
    async def collect_mails_content(self, mail_urls):
        async with aiohttp.ClientSession() as session:
            return await asyncio.gather(
                *[self.get_mail_content(url, session) for url in mail_urls]
            )
